...
SELECT COUNT(*) AS Ratings_Rows_Count
FROM ratings;
...
WITH Genre_Summery AS(
SELECT movie_id,
       COUNT(genre) AS Genre_Count 
FROM genre
GROUP BY movie_id
)
SELECT Count(DISTINCT movie_id) as Single_Genre_Movie
FROM Genre_Summery
WHERE Genre_Count = 1;
...
SELECT 
      ...
      MIN(median_rating) AS Max_Median_Rating
FROM ratings
)
SELECT *
FROM Min_Max_Rating_Table;
...
WITH Movie_Count_Median_Rating AS(
SELECT median_rating, 
	   COUNT(movie_id) as movie_count
FROM ratings
GROUP BY median_rating
)
SELECT median_rating, movie_count
FROM Movie_Count_Median_Rating
...
WITH Hit_movies_count AS(
SELECT m.production_company, 
	   COUNT(m.id) as Movie_Count,
       RANK() OVER (ORDER BY COUNT(m.id) DESC) as Production_Company_Rank
FROM movie as m
LEFT JOIN ratings as r ON m.id = r.movie_id
...
SELECT 
    ROUND(german_movie_votes / NULLIF(german_movie_count, 0), 2) AS German_Votes_Per_Movie,
    ROUND(italian_movie_votes / NULLIF(italian_movie_count, 0), 2) AS Italian_Votes_Per_Movie
FROM 
    votes_summary;
...
WITH Name_Table_Null_Col_Values AS(
SELECT 
...
SELECT *
FROM Name_Table_Null_Col_Values;
...
Top_3_Director AS (
    SELECT n.name AS director_names, COUNT(m.id) as Movie_Count
    FROM names AS n
    INNER JOIN director_mapping AS dm ON n.id = dm.name_id
    INNER JOIN movie AS m ON dm.movie_id = m.id
    INNER JOIN ratings AS r ON m.id = r.movie_id
    INNER JOIN genre AS g ON m.id = g.movie_id
    WHERE g.genre IN (SELECT DISTINCT genre FROM Top_Rated_Movie WHERE Movie_Rank <= 3)
    AND r.avg_rating > 8
    GROUP BY n.name
    ORDER BY Movie_Count DESC
    LIMIT 3
...
WITH Top_3_Productions AS
(
SELECT m.production_company, 
	   SUM(r.total_votes) as Votes_Count,
       ROW_NUMBER() OVER (ORDER BY SUM(r.total_votes) DESC) AS Prod_Com_Rank
FROM movie as m
JOIN ratings as r ON m.id = r.movie_id
WHERE production_company IS NOT NULL
GROUP BY production_company
)
SELECT * FROM Top_3_Productions
WHERE Prod_Com_Rank <= 3;
...
INNER JOIN ratings as r
ON m.id = r.movie_id
WHERE category = 'actress' and LOWER(country) LIKE '%india%'
...
SELECT m.title, g.genre,
	   CASE 
           WHEN r.avg_rating >= 8 THEN 'Superhit_Movies'
           WHEN r.avg_rating BETWEEN 7 AND 8 THEN 'Hit_Movies'
           WHEN r.avg_rating BETWEEN 5 AND 7 THEN 'One_Time_Watch_Movies'
           WHEN r.avg_rating < 5 THEN 'Flop_Movies'
...
RANK() OVER (ORDER BY r.total_votes DESC) AS Actress_Rank
FROM names AS n
...
GROUP BY Actress_Name, Total_Votes 
)
